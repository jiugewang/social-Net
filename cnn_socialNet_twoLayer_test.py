...
def get_test_data():
    ...
    # file_path_community = './0814data/test/community-standard.txt'
    # file_path_network = './0814data/test/community-edges.txt'
...
test_data, count0, count1 = get_test_data()
print(test_data[0])
print("所有边的个数", len(test_data))
...
for i in range(len(test_data)):
    test_x_1.append(test_data[i][0])
    test_x_2.append(test_data[i][1])
    test_y.append(test_data[i][2])
...
def validate(test_x, tfsavepath):
    ...
    writer = tf.summary.FileWriter('./logdiffjump1test')
    with tf.Session() as sess:
        ...
        for i in range(len(test_x_1)):
            tmp_jump1 = []
            tmp_jump1.append(test_x_1[i])
            tmp_jump2 = []
            tmp_jump2.append(test_x_2[i])
            res = sess.run([predict, tf.argmax(output, 1)], feed_dict={x1_data: tmp_jump1, x2_data: tmp_jump2, keep_prob_5: 1.0, keep_prob_75: 1.0})
            # print(res)
            # print(res[1][0])
            image = tf.convert_to_tensor(test_x_2[i])
        ...
        with open('./0814data/result.txt', 'w') as f:
            f.writelines("right0:{}\n".format(right0))
            f.write("right1:{}\n".format(right1))
            f.write("error0:{}\n".format(error0))
            f.write("error1:{}\n".format(error1))
...
if __name__ == '__main__':
    validate(test_x_1, test_x_2, './checkpoint1/social.ckpt')
