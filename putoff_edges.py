...
def simplify_network(g):
    ...
    saver_path = './checkpoint1/social.ckpt'
    saver = tf.train.Saver()
    with tf.Session() as sess:
        saver.restore(sess, saver_path)
        for i in range(len(data)):
            tmp_matrix = []
            tmp_matrix.append(data[i])
            # 根据模型预测边是否为社区间的边
            res = sess.run([predict, tf.argmax(output, 1)],
                           feed_dict={x_data: tmp_matrix, keep_prob_5: 1.0, keep_prob_75: 1.0})
            global the_flag
            if res[1][0] == 0:
                the_flag = 1
            elif res[1][0] == 1:
