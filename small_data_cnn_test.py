...
def get_test_data():
    ...
    for j in range(len(edges)):
        ...
        resize_image = tf.image.resize_images(image, [SIZE32, SIZE32], method=3)
    ...
    sess.close()
    return my_graph, train_x_y, flag0_count, flag1_count


G, test_data, count0, count1 = get_test_data()
# print(G.edges())
# G.remove_edge(str(17), str(18))
# print(G.edges())
# print(test_data[0])
...
x_data = tf.placeholder(tf.float32, [None, SIZE32, SIZE32, 3])
...
def cnn_layer(classnum):
    ...
    # 全连接层
    Wf = weight_variable([2 * 2 * 256, 1024])
    bf = bias_variable([1024])
    drop3_flat = tf.reshape(pool4, [-1, 2 * 2 * 256])
...
if __name__ == '__main__':
    validate(test_x, './checkpoint1/social.ckpt')
