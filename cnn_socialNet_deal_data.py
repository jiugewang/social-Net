...
def get_jump1_3dimension_different_size_matrix(mygraph, edge):
    """
    获取一条边端点2跳的矩阵模型
    :param mygraph:网络
    :param edge:网络中的某条边
...
def get_jump2_1dimension_different_size_matrix(mygraph, edge):
    """
    获取一条边端点2跳的矩阵模型
    :param mygraph:网络
    :param edge:网络中的某条边
    ...
    # 构造矩阵
    matrix = np.zeros((source_list_size, target_list_size, 1))
    for row in range(source_list_size):
        for clown in range(target_list_size):
            if source_node_list[row] in intersection_set and target_node_list[clown] in intersection_set:
                matrix[row][clown] = 255
            elif (source_node_list[row], target_node_list[clown]) in edges:
                matrix[row][clown] = 128
            else:
                matrix[row][clown] = 0
...
# 128矩阵
def get_jump2_3dimension_128matrix(G, edge):
    """
    获取一条边端点2跳的矩阵模型
    :param G:网络
    :param edge:网络中的某条边
    ...
    source_node_set = set()
    target_node_set = set()
    for source_nbr in G[source_node]:
        if source_nbr == target_node:
            pass
        else:
            source_node_set.add(source_nbr)
            for source_nbr1 in G[source_nbr]:
                source_node_set.add(source_nbr1)
    if source_node in source_node_set:
        source_node_set.remove(source_node)
    if target_node in source_node_set:
        source_node_set.remove(target_node)
    for target_nbr in G[target_node]:
        if target_nbr == source_node:
            pass
        else:
            target_node_set.add(target_nbr)
            for target_nbr1 in G[target_nbr]:
                target_node_set.add(target_nbr1)
    if source_node in target_node_set:
        target_node_set.remove(source_node)
    if target_node in target_node_set:
    ...
    while len(source_node_list) < SIZE128:
        source_node_list.append(-1)
    while len(target_node_list) < SIZE128:
        target_node_list.append(-1)

    if len(source_node_list) > SIZE128:
        source_node_list = source_node_list[0:SIZE128]
    if len(target_node_list) > SIZE128:
        target_node_list = target_node_list[0:SIZE128]

    # 构造SIZE*SIZE*3的矩阵
    matrix = np.zeros((SIZE128, SIZE128, 3))

    # print(matrix)
    # 迭代邻居结点
    for row in range(SIZE128):
        for clown in range(SIZE128):
            if source_node_list[row] == -1 or target_node_list[clown] == -1:
                matrix[row][clown] = 0
            elif source_node_list[row] in intersection_set and target_node_list[clown] in intersection_set:
                matrix[row][clown][1] = 255
    ...
    # return (source_node_set, target_node_set, intersection_set, source_node_list, target_node_list), matrix
    return matrix
